...
def debug_msg(currentModule, message, msgType):
    if DEBUG_LEVEL != 0 and msgType <= DEBUG_LEVEL:
...
def meta_path():
    ...
    try:
        ...
        if not msf_path.endswith("/"):
            msf_path = f"{msf_path}/"
        if os.path.isfile(f"{msf_path}msfconsole"):
            trigger = 1

        # if we are using just the standard path for msfconsole
        if os.path.isfile("/usr/bin/msfconsole") and trigger == 0:
            msf_path = "/usr/bin/"
            trigger = 1

        # specific for backbox linux
        if (
            os.path.isfile("/opt/metasploit-framework/msfconsole")
            and trigger == 0
        ):
        ...
        if (
            os.path.isfile("/opt/metasploit/apps/pro/msf3/msfconsole")
            and trigger == 0
        ):
            msf_path = ""
            trigger = 1
        ...
        if os.path.isfile("/opt/framework3/msf3/msfconsole") and trigger == 0:
            msf_path = "/opt/framework3/msf3/"
            trigger = 1
        if os.path.isfile("/opt/framework/msf3/msfconsole") and trigger == 0:
            msf_path = "/opt/framework/msf3/"
            trigger = 1
        if os.path.isfile("/opt/metasploit/msf3/msfconsole") and trigger == 0:
        ...
        if (
            os.path.isfile("/pentest/exploitation/metasploit/msfconsole")
            and trigger == 0
        ):
        ...
        if (
            os.path.isfile("/usr/share/metasploit-framework/msfconsole")
            and trigger == 0
        ):
        ...
        print_status(f"Something went wrong. Printing error: {str(e)}")
...
def check_ports(filename, port):
    ...
    data = fileopen.read()
    return bool(match := re.search(f"LPORT {port}", data))
...
def validateCIDRBlock(b):
    ...
    if (int(subnet) < 1) or (int(subnet) > 32):
        print(f"Error: subnet {str(subnet)} wrong size.")
        return False
...
def get_sql_port(host):
    ...
    try:
        ...
        try:
            ...
            return_host = f"{host}:{sql_port}"
            if return_host != f":{sql_port}":
                return f"{host}:{sql_port}"
...
def check_backbox():
    if os.path.isfile("/etc/issue"):
        ...
        return "BackBox" if "BackBox" in backboxdata else "Non-BackBox"
    else:
...
def fetch_template():
    fileopen = open(f"{userconfigpath}site.template").readlines()
    for line in fileopen:
        line = line.rstrip()
        if match := re.search("URL=", line):
            line = line.split("=")
            return line[1]
...
def powershell_encodedcommand(ps_attack):
    ...
    return (
        'powershell -w 1 -C "sv {0} -;sv {1} ec;sv {2} ((gv {3}).value.toString()+(gv {4}).value.toString());powershell (gv {5}).value.toString() \''.format(
            ran1, ran2, ran3, ran1, ran2, ran3
        )
        + ps_attack
        + '\'"'
    )
